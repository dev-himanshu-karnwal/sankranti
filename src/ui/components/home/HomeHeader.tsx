...
export default function HomeHeader({ onMenuToggle, onCartToggle }: HomeHeaderProps) {
  const navigate = useNavigate();
  return (
    <>
      <div className="max-[430px]:block hidden"><PanelLayout isShowDeliveryOption={true} /></div>
      <header className="hidden min-[431px]:flex items-center justify-between px-4 lg:px-10 py-3 w-full bg-surface-subtle shadow-sm z-50 overflow-x-hidden">
        <div className="flex-1 flex items-center justify-start flex-shrink-0">
          <img 
            src="/images/sankranti_indian_kitchen.png" alt="Sankranti Logo" 
            className="w-[85px] h-auto object-contain cursor-pointer" onClick={() => navigate('/home')} 
          />
        </div>
        <nav className="flex items-center gap-4 lg:gap-8 justify-center flex-shrink-0 overflow-x-auto no-scrollbar">
          <Link to="/home" className="text-[12px] font-bold text-primary tracking-wide leading-none">HOME</Link>
        ...
        <div className="flex items-center gap-3 lg:gap-5 flex-1 justify-end flex-shrink-0">
          <AppButton onClick={onMenuToggle} className="!w-auto !p-0 hover:bg-transparent" aria-label="Profile Menu">
            <svg width="20" height="20" viewBox="0 0 23 27" fill="none"><path d="M11.5001 0C15.3048 0 18.4001 3.09534 18.4001 6.9C18.4001 10.7047 15.3048 13.8 11.5001 13.8C7.69549 13.8 4.60015 10.7047 4.60015 6.9C4.60015 3.09534 7.69549 0 11.5001 0ZM11.5001 12.2667C14.4593 12.2667 16.8668 9.85918 16.8668 6.9C16.8668 3.94082 14.4593 1.53333 11.5001 1.53333C8.54097 1.53333 6.13348 3.94082 6.13348 6.9C6.13348 9.85918 8.54097 12.2667 11.5001 12.2667Z" fill="var(--color-secondary)"/><path d="M2.91507 18.3067C4.80414 16.3886 7.30843 15.3323 9.96667 15.3323H13.0333C15.6915 15.3323 18.1959 16.3886 20.0849 18.3067C21.9647 20.2154 23 22.7349 23 25.4012C23 25.8246 22.6567 26.1678 22.2333 26.1678H0.766666C0.343264 26.1678 0 25.8246 0 25.4012C0 22.7349 1.03526 20.2154 2.91507 18.3067ZM21.4329 24.6345C21.0488 20.2857 17.4283 16.8656 13.0333 16.8656H9.96667C5.57167 16.8656 1.95117 20.2857 1.56707 24.6345H21.4329Z" fill="var(--color-secondary)"/></svg>
          </AppButton>
          ...
          <div className="max-[430px]:block hidden">
            <AppButton className="!w-auto border border-secondary rounded-full px-4 py-[6px] text-[10px] font-bold text-text-primary tracking-wider bg-transparent hover:bg-black/[0.02] transition-colors !justify-center">PICKUP OR DELIVERY</AppButton>
          </div>
        </div>
      </header>
    </>
