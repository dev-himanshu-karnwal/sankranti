  ...
  return (
    <div className="md:hidden flex flex-col h-screen w-full max-w-[428px] mx-auto overflow-hidden bg-white relative z-10 shadow-xl">
      <div className="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden flex flex-col items-center">
          ...
          {categories.map((cat, idx) => (
            <Link to={`/category/${cat.title.toLowerCase()}`} key={idx} className="flex flex-col items-center gap-3 group">
              <div className="w-[88px] h-[88px] rounded-full bg-surface-muted flex items-center justify-center p-2 group-hover:scale-105 transition-transform">
                <img src={cat.image} alt={cat.title} className="w-[90%] h-[90%] object-contain mix-blend-multiply drop-shadow-sm transition-transform group-hover:scale-110" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
              </div>
        ...
        <div className="w-[calc(100%-48px)] mx-6 bg-white rounded-[16px] shadow-sm p-3.5 mb-10 border border-border-subtle flex gap-4 cursor-pointer">
          <div className="w-[120px] bg-surface-muted rounded-[10px] flex items-center justify-center pt-2 pb-1 relative overflow-hidden"><img src="/images/protein_bowl.png" alt="Bowl" className="w-[110%] ml-2 h-auto object-contain drop-shadow-md mix-blend-multiply relative" /></div>
          <div className="flex flex-col flex-1 py-1 pr-1">
            <h3 className="text-[14px] font-semibold text-text-primary mb-1">Protein Bowl</h3>
            <p className="text-[8.5px] font-medium text-text-secondary leading-[1.4] mb-4 pr-1">Ghee rice base. Pick a protein, curry sauce, and toppings. Fryums on the side.</p>
            <div className="flex items-center justify-between mt-auto"><span className="text-[14px] font-bold text-text-primary">$14.95</span><Button variant="secondary" className="!w-auto !h-[26px] !py-0 !px-5 !text-[10px] !font-medium !rounded-[6px]">Add Item</Button></div>
          </div>
        </div>
      </div>
      <div className="flex-shrink-0 bg-white border-t border-border-subtle pb-[env(safe-area-inset-bottom)] relative z-50"><PanelFooter /></div>
    </div>
  );
}
