    ...
    >
      <div className="h-full flex flex-col pt-10 overflow-x-hidden overflow-y-auto relative [&::-webkit-scrollbar]:hidden">
        ...
        <div className="flex items-center justify-between px-6 mb-5 flex-shrink-0 relative z-20 min-h-[44px]">
          ...
          <button
            onClick={onClose}
            className="text-secondary hover:scale-110 transition-transform p-2"
          >
            <CloseIcon />
          </button>
        </div>
        ...
        <div
          ...
          style={{
            transform:
              view === "rewards" ? "translateX(-50%)" : "translateX(0%)",
          }}
        >
          <div className="w-1/2 flex flex-col">
            <div className="flex flex-col justify-center items-center gap-[14px] mb-12 px-8">
              <Button
                variant="secondary"
                className="!py-[12px] !w-[160px] uppercase text-[12px] font-bold tracking-wide"
              >
                Create Account
              </Button>
              <Button
            ...
            <ul className="flex flex-col">
              {MAIN_MENU_ITEMS.map((item, index) => (
                <li
                  key={index}
                  className="border-t border-[#E8D4D8] first:border-t-0 border-b border-[#E8D4D8] -mb-[1px]"
                >
                  <button
          ...
          <div className="w-1/2 flex flex-col">
            <div className="flex flex-col items-center mb-8 drop-shadow-sm px-8">
              <span className="text-[18px] text-text-primary text-center mb-0.5">
                You've Got
              </span>
              <span className="text-[32px] font-bold text-text-primary text-center leading-none tracking-tight">
                10 POINTS
              </span>
            </div>
...
function RewardFeatureItem({
  ...
  return (
    <li className={`border-b border-[#E8D4D8] ${isTop ? "border-t" : ""}`}>
      <button
        className={`w-full flex flex-col items-start px-8 text-left transition-colors hover:bg-black/[0.02] ${subtitle ? "py-[14px]" : "py-5"}`}
      >
        <span className="text-[11px] font-bold text-text-secondary uppercase tracking-wide">
          {title}
        </span>
        {subtitle && (
          <span className="text-[11px] text-text-secondary font-medium leading-normal opacity-70 mt-0.5">
            {subtitle}
          </span>
        )}
      </button>
...
function Footer({ className = "" }: { className?: string }) {
  return (
    <div className={`flex flex-col items-start gap-4 ${className}`}>
      <div className="text-[11px] font-medium text-text-secondary opacity-60 tracking-wide">
        ©Copyright2026 Sankranti
      </div>
      <div className="flex flex-col gap-3">
        <Link
          to="#"
          className="text-[10px] font-bold text-secondary uppercase tracking-widest hover:opacity-75 transition-opacity"
        >
          Privacy Policy
        </Link>
        <Link
      ...
      <div className="text-[11px] font-medium text-text-secondary opacity-60 tracking-wide mt-1">
        App version: 1.3
      </div>
    </div>
  );
