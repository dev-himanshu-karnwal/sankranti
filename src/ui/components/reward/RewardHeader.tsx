...
export default function RewardHeader({ onMenuToggle, onCartToggle }: RewardHeaderProps) {
  const navigate = useNavigate();
  return (
    <header className="hidden md:flex items-center justify-between px-10 py-3 w-full bg-white shadow-sm z-50 sticky top-0">
      <div className="flex-1 flex items-center justify-start cursor-pointer" onClick={() => navigate('/home')}>
      ...
      <nav className="flex items-center gap-8 justify-center flex-shrink-0">
        ...
        <Link to="/reward" className="text-[12px] font-bold text-primary tracking-wide leading-none">REWARDS</Link>
      </nav>
      <div className="flex items-center gap-5 flex-1 justify-end">
        <AppButton onClick={onMenuToggle} className="!w-auto !p-0 hover:bg-transparent" aria-label="Profile">
