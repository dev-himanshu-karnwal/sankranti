    ...
    <div className="flex gap-4 justify-between w-full">
      {Array.from({ length }).map((_, i) => (
        <input
          ...
          onPaste={handlePaste}
          className="w-[60px] h-[60px] text-center text-xl font-medium bg-white border border-[#D5D5D5] rounded-2xl outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors text-text-primary"
        />
      ))}
    </div>
  );
